...
class AgentNodes:
    ...
    def create_final_answer(self, state: AgentState) -> Dict[str, Any]:
        ...
        if is_solved:
            ...
            # 去除可能的外层 markdown 代码块
            final_answer = final_answer.strip()
            if final_answer.startswith("```markdown"):
                final_answer = final_answer[11:].strip()  # 去掉 ```markdown
            if final_answer.startswith("```"):
                final_answer = final_answer[3:].strip()   # 去掉开头的 ```
            if final_answer.endswith("```"):
                final_answer = final_answer[:-3].strip()  # 去掉结尾的 ```
        else:
            # 生成失败报告
            final_answer = f"""## 解答报告
