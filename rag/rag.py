...
class RagService(object):
    def __init__(self):
        ...
        self.prompt_template = ChatPromptTemplate.from_messages([
            ...
            ("system", "我将为你提供用户提问相关资料: {context}。基于此解答用户所询问的题目，并且给出核心思路的分析"),
            ("user", "请回答用户待求解的题目: {input}")
        ])
    ...
    def __get_chain(self):
        ...
        def __format_document(docs: List[Document]):
            if not docs:
                return "无相关参考资料"
        ...
        chain = {"input": RunnablePassthrough(),
                 "context": retriever | __format_document} | self.prompt_template | self.chat_model | StrOutputParser()
        return chain
